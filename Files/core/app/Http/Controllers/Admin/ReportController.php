...
class ReportController extends Controller
{
    ...
    public function commissionsDeposit()
    {
        ...
        $logs = CommissionLog::searchable(['trx','userTo:username'])->where('commission_type','deposit_commission')->with(['userTo','userFrom'])->latest()->paginate(getPaginate());
        return view('admin.reports.commission_log', compact('pageTitle', 'logs'));
    }
    public function commissionsBuy()
    {
        $pageTitle = 'Buy Lottery Commissions';
        $logs = CommissionLog::searchable(['trx','userTo:username'])->where('commission_type','buy_commission')->with(['userTo','userFrom'])->latest()->paginate(getPaginate());
        return view('admin.reports.commission_log', compact('pageTitle', 'logs'));
    }
    public function commissionsWin()
    {
        $pageTitle = 'Win Lottery Commissions';
        $logs = CommissionLog::searchable(['trx','userTo:username'])->where('commission_type','win_commission')->with(['userTo','userFrom'])->latest()->paginate(getPaginate());
        return view('admin.reports.commission_log', compact('pageTitle', 'logs'));
    }
    ...
    public function tickets()
    {
        ...
        $tickets = Ticket::orderBy('id','desc')->with('user','lottery','phase')->paginate(getPaginate());
